...
def show_venue(venue_id):  
  ...
  for show in shows:    
    ...
    if date_obj < today_str:      
      print("the concert is in the past")
      past_shows.append({
        'artist_id': show[1],
        'artist_name': show[2],
...
def shows():  
  ...
  showdata = []  
  for show in shows:       
    print('Artist: {}'.format(show[1].name, show[1].id) + ', Venue: {}'.format(show[0].name, show[0].id))
    show_start_time = show[2].start_time
    show_start_time_obj = show_start_time.strftime('%Y-%m-%d %H:%M:%S')       
    showdata.append({"venue_id": show[0].id, "venue_name": show[0].name, "artist_id": show[1].id, "artist_name": show[1].name, "artist_image_link": show[1].image_link, "start_time": show_start_time_obj})        
    print(show[1].image_link)
  print(showdata)
